...
## Parts that come with each job

222 of the 245 catalog jobs carry the parts they usually need (the rest are
diagnosis, tests and labor-only services). Adding a job to a ticket adds those
parts with **no price and no part number**, tied to the job:

- each line says **PRICE NEEDED** and **SHOP SEED — VERIFY FITMENT**; the
  ticket says **N PARTS NOT PRICED YET** and its total leaves them out;
- the customer's estimate prints **price to follow** for them, never $0.00;
- a ticket is not invoiced, printed as an invoice, paid or closed while a part
  has no price - enter the price when you order it (0 if it is free) and
  remove the ones you did not use;
- oil-change packages list their oil and filter as **INCLUDED**, off the shelf;
- removing a job offers to remove its parts that are not priced or ordered;
- **Services → Parts** on any job edits that job's list for this shop (Reset
  goes back to the seed). The lists are generated by `seed_job_parts.py`.
...
node testjobparts.cjs  # parts that come with each job: unpriced lines, never invoiced at $0
...
Tag a version (`git tag v2.9.1 && git push origin v2.9.1`) and CI runs every
